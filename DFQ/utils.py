...
def test(model, loader, prefix='Model', device="cuda"):
    ...
    print('[{}]: Accuracy of the network on the 50000 test images: {:.2f} %'.format(prefix, 100 * correct / total))
...
# Note 这里的quantize是一个fakequantize，就是做了quant然后再做反量化
...
class UniformQuantize(InplaceFunction):
    ...
    @staticmethod
    def forward(ctx, input, num_bits=8, min_value=None, max_value=None, inplace=False, symmetric=False,
                num_chunks=None):
...
class QuantMeasure(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            # https://github.com/jakc4103/DFQ/blob/6f15805cfdbf2769275defd54728df0a5d30dbc6/utils/layer_transform.py#L347
            # The origin DFQ determin the max and min value depend on the mean and variation of batchnorm
            # TODO the min_value and max_value for now is just depended on input data
...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1, bias=True, num_bits=8, num_bits_act=8, num_bits_bias=16,
                 momentum=0.1):
    ...
    def merge_scale_to_weight(self):
        ...
        if hasattr(self, 'scale') and self.scale is not None:
            weight, bias = self.merge_scale(self.weight.detach(),
                                            self.bias.detach() if self.bias is not None else self.bias, self.scale)
            self.weight.data.copy_(weight)
    ...
    def merge_scale_prev(self, weight, scale_prev):
        ...
        for g in range(self.groups):
            sweight[g * step:(g + 1) * step] = weight[g * step:(g + 1) * step] / scale_prev[:,
                                                                                 g * step_s:(g + 1) * step_s]
...
class QLinear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True, num_bits=8, num_bits_act=8, num_bits_bias=16,
                 momentum=0.1):
